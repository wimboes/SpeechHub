...
class PTBInput(object):
    ...
    def __init__(self, config, data, name=None):
        ...
        self.input_data, self.targets = reader_LDA.ptb_producer(data, batch_size, num_steps, name=name)
...
def get_loss_function(output, softmax_w, softmax_b, targets, topic_matrix, batch_size, num_steps, is_training):
    ...
    if FLAGS.loss_function == 'sampled_softmax':
        if is_training:
            ...
            logits = tf.matmul(output, softmax_w) + softmax_btopic_matrix
            loss = tf.nn.sparse_softmax_cross_entropy_with_logits(logits, tf.reshape(targets, [-1]), name=None)
            return tf.reduce_sum(loss) / batch_size
...
def main(_):
    ...
    raw_data = reader_LDA.ptb_raw_data_lda(FLAGS.data_path, FLAGS.text_data, FLAGS.vocab_size)
    ...
    with tf.Graph().as_default():    
        ...
        param_train_np = np.array([['init_scale',config.init_scale], ['learning_rate', config.learning_rate],
                                   ...
                                   ['max_max_epoch', config.max_max_epoch],['keep_prob', config.keep_prob], ['nb_topcis', FLAGS.nb_topics],

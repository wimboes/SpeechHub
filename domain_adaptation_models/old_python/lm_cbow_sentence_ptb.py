...
import reader_cbow_sentence_ptb
...
output_path = os.path.join(general_path,'output/output_cbow_sentence_ptb')
...
flags.DEFINE_integer("max_max_epoch", 15, "max_max_epoch")
...
class ptb_input(object):
    def __init__(self, config, max_length, word_to_id, data, name=None):
        ...
        self.input_data, self.targets, _, self.history, self.average_sentence_length = reader_cbow_sentence_ptb.ptb_producer(data, batch_size, max_length, num_history, word_to_id, name=name)

class ptb_cbow_sentence_model(object):
...
def main(_):
    ...
    raw_data = reader_cbow_sentence_ptb.ptb_raw_data(FLAGS.data_path)
    ...
    with tf.Graph().as_default():
        ...
        with tf.name_scope("train"):
            train_input = ptb_input(config=config, max_length=max_length, word_to_id=word_to_id, data=train_data, name="train_input")
            with tf.variable_scope("model", reuse=None, initializer=initializer):
                m = ptb_cbow_sentence_model(is_training=True, config=config, input_=train_input)

        with tf.name_scope("valid"):
            valid_input = ptb_input(config=config, max_length=max_length, word_to_id=word_to_id, data=valid_data, name="valid_input")
            with tf.variable_scope("model", reuse=True, initializer=initializer):
                mvalid = ptb_cbow_sentence_model(is_training=False, config=config, input_=valid_input)

        with tf.name_scope("test"):
            test_input = ptb_input(config=eval_config, max_length=max_length, word_to_id=word_to_id, data=test_data, name="test_input")
            with tf.variable_scope("model", reuse=True, initializer=initializer):
                mtest = ptb_cbow_sentence_model(is_training=False, config=eval_config, input_=test_input)

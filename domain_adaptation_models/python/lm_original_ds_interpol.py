...
flags.DEFINE_string("eval_name",'kort.txt',"eval_name")
...
def run_epoch(session, model, eval_op=None, verbose=False, epoch_nb = 0):
    ...
    with open((FLAGS.save_path + '/' + FLAGS.test_name + '_' + str(FLAGS.num_run) + '/eval2' +'.txt'), "w") as f:
        f.write('\n')           
        for step in range(model.input.epoch_size):
            ...
            labels = vals["temp2"]
            probs = vals['temp4']
            probs= np.log10(np.exp(-probs))            
            
            unk_mask = [1 if labels[i]== model.input.unk_id else 0 for i in range(len(data))]
            sentence = [reverse_dict[data[i]].encode('utf-8') for i in range(len(data))]
            sentence.append(reverse_dict[model.input.eos_id].encode('utf-8'))            
                
            logprob = sum([probs[i] for i in range(0,len(probs)) if unk_mask[i]==0])
            unks = sum(unk_mask)
            ...
            if step % (model.input.epoch_size // 2) == 0:
        ...
        f.write('file ' + FLAGS.data_path + '/eval2: '+ str(nb_sentences) + ' sentences, ' + str(nb_words) + ' words, ' + str(nb_unks) + ' OOVs \n')   

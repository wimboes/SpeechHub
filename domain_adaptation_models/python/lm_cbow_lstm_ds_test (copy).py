...
class ds_cbow_sentence_model(object):
    ...
    def __init__(self, is_training, config, input_):
        ...
        if config['cbow_position'] == "soft":
            loss = get_loss_function(output_soft, softmax_w_soft, softmax_b_soft, labels, input_, is_training)
            self._temp1,self._temp2,self._temp3 = get_N_most_probable_words(output_soft, softmax_w_soft, softmax_b_soft, labels, data, input_)
            _,_,self._temp4 = get_probability(output_soft, softmax_w_soft, softmax_b_soft, labels, data, input_)
        elif config['cbow_position'] == "lstm":
            loss = get_loss_function(output_lstm, softmax_w_lstm, softmax_b_lstm, labels, input_, is_training)
...
def main(_):
    ...
    with tf.Graph().as_default():
        with tf.name_scope("test"):
            eval_data = reader.ds_data_with_history(eval_config['batch_size'], eval_config['num_history'], FLAGS.data_path, FLAGS.eval_name)

...
output_path = os.path.join(general_path,'output_original')
...
class ds_input(object):
    def __init__(self, config, data, name=None):
        ...
        self.input_data, self.targets = reader_original.ds_producer(data, batch_size, num_steps, name=name)


class ds_model(object):
...
def main(_):
    ...
    raw_data = reader_original.ds_raw_data(FLAGS.data_path)
    ...
    with tf.Graph().as_default():
        ...
        with tf.name_scope("train"):
            ...
            with tf.variable_scope("model", reuse=None, initializer=initializer):
                m = ds_model(is_training=True, config=config, input_=train_input)
        ...
        with tf.name_scope("valid"):
            ...
            with tf.variable_scope("model", reuse=True, initializer=initializer):
                mvalid = ds_model(is_training=False, config=config, input_=valid_input)
            tf.scalar_summary("Validation Loss", mvalid.cost)

        with tf.name_scope("test"):
            test_input = ds_input(config=config, data=test_data, name="test_input")
            with tf.variable_scope("model", reuse=True, initializer=initializer):
                mtest = ds_model(is_training=False, config=eval_config, input_=test_input)

...
import reader_original_ptb

python_path = os.path.abspath(os.getcwd())
general_path = os.path.split(python_path)[0]
input_path = os.path.join(general_path,'input')
output_path = os.path.join(general_path,'output/output_original_ptb')
...
class ptb_input(object):
    def __init__(self, config, data, name=None):
        ...
        self.input_data, self.targets = reader_original_ptb.ptb_producer(data, batch_size, num_steps, name=name)


class ptb_original_model(object):
...
def main(_):
    ...
    raw_data = reader_original_ptb.ptb_raw_data(FLAGS.data_path)
    ...
    with tf.Graph().as_default():
        ...
        with tf.name_scope("train"):
            train_input = ptb_input(config=config, data=train_data, name="train_input")
            with tf.variable_scope("model", reuse=None, initializer=initializer):
                m = ptb_original_model(is_training=True, config=config, input_=train_input)
            tf.scalar_summary("Training Loss", m.cost)
            tf.scalar_summary("Learning Rate", m.lr)

        with tf.name_scope("valid"):
            valid_input = ptb_input(config=config, data=valid_data, name="valid_input")
            with tf.variable_scope("model", reuse=True, initializer=initializer):
                mvalid = ptb_original_model(is_training=False, config=config, input_=valid_input)
            tf.scalar_summary("Validation Loss", mvalid.cost)

        with tf.name_scope("test"):
            test_input = ptb_input(config=config, data=test_data, name="test_input")
            with tf.variable_scope("model", reuse=True, initializer=initializer):
                mtest = ptb_original_model(is_training=False, config=eval_config, input_=test_input)

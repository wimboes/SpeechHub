...
output_path = os.path.join(general_path,'output/output_topic_ptb')
...
flags.DEFINE_integer("max_max_epoch_reg", 15, "max_max_epoch_reg")
...
flags.DEFINE_integer("max_max_epoch_lda", 15, "max_max_epoch_lda")
...
class ptb_input(object):
    def __init__(self, config, max_length, length_array, data, name=None):
        ...
        self.input_data, self.targets, self.sentence_lengths_batch = reader_topic_ptb.ptb_producer(data, batch_size, length_array, max_length, name=None)

class ptb_topic_model(object):
...
def main(_):
    ...
    raw_data =  reader_topic_ptb.ptb_raw_data(FLAGS.data_path)
    ...
    with tf.Graph().as_default():    
        ...
        with tf.name_scope("train"):
            train_input = ptb_input(config=config, max_length=max_length, length_array=train_length_array, data=train_data, name="train_input")
            with tf.variable_scope("model", reuse=None):
            	m = ptb_topic_model(is_training=True, config=config, input_ = train_input, topic_matrix = topic_matrix, initializer_reg = initializer_reg, initializer_lda = initializer_lda)
            tf.scalar_summary("Training Loss", m.cost)
            tf.scalar_summary("Learning Rate", m.lr)

        with tf.name_scope("valid"):
            valid_input = ptb_input(config=config, max_length=max_length, length_array=valid_length_array, data=valid_data, name="valid_input")
            with tf.variable_scope("model", reuse=True):
                mvalid = ptb_topic_model(is_training=False, config=config, input_ = valid_input, topic_matrix = topic_matrix, initializer_reg = initializer_reg, initializer_lda = initializer_lda)
            tf.scalar_summary("Validation Loss", mvalid.cost)

        with tf.name_scope("test"):
            test_input = ptb_input(config=eval_config, max_length=max_length, length_array=test_length_array, data=test_data, name="test_input")
            with tf.variable_scope("model", reuse=True):
                mtest = ptb_topic_model(is_training=False, config=eval_config, input_ = test_input, topic_matrix = topic_matrix, initializer_reg = initializer_reg, initializer_lda = initializer_lda) 

...
import reader_cbow_sentence_ds
...
output_path = os.path.join(general_path,'output/output_cbow_sentence_ds')
...
class ds_input(object):
    def __init__(self, config, max_length, word_to_id, data, name=None):
        ...
        self.input_data, self.targets, _, self.history, self.average_sentence_length = reader_cbow_sentence_ds.ds_producer(data, batch_size, max_length, num_history, word_to_id, name=name)
...
def get_loss_function(output, softmax_w, softmax_b, targets, batch_size, is_training,unk_id, vocab_size):
    ...
    #masking of vocab_size id id's (always) and unk_id (only during testing)
...
def main(_):
    ...
    raw_data = reader_cbow_sentence_ds.ds_raw_data(FLAGS.data_path)
    ...
    with tf.Graph().as_default():
        ...
        with tf.name_scope("train"):
            ...
            with tf.variable_scope("model", reuse=None, initializer=initializer):
                m = ds_cbow_sentence_model(is_training=True, config=config, input_=train_input)
        ...
        with tf.name_scope("valid"):
            ...
            with tf.variable_scope("model", reuse=True, initializer=initializer):
                mvalid = ds_cbow_sentence_model(is_training=False, config=config, input_=valid_input)

        with tf.name_scope("test"):
            test_input = ds_input(config=eval_config, max_length=max_length, word_to_id=word_to_id, data=test_data, name="test_input")
            with tf.variable_scope("model", reuse=True, initializer=initializer):
                mtest = ds_cbow_sentence_model(is_training=False, config=eval_config, input_=test_input)

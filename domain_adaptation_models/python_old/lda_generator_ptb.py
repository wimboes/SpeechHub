...
def lda_generate_model(sentences_per_document, nb_topics, data_path, lda_save_path, dict_save_path):
    ...
    corpora.MmCorpus.serialize('bow.ptb.mm',corpus)
    mm = corpora.MmCorpus('bow.ptb.mm')
    tfidf = models.TfidfModel(mm,id2word=dictionary,normalize=True)
    corpora.MmCorpus.serialize('tfidf.ptb.mm', tfidf[mm], progress_cnt=10000)

    mm = corpora.MmCorpus('tfidf.ptb.mm')
...
lda_save_path = os.path.join(data_path, "lda.ptb.model")
dict_save_path = os.path.join(data_path, "dictionary.ptb.dict")

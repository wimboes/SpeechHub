...
class PTBInput(object):
    ...
    def __init__(self, config, data, name=None):
        ...
        self.steps_in_sentence =  (len(data[0])-1)//num_steps
        self.epoch_size = self.steps_in_sentence * (len(data)// batch_size)
        self.input_data, self.targets = reader_sentence_var.ptb_producer(data, batch_size, num_steps, name=name)
...
class PTBModel(object):
    ...
    def __init__(self, is_training, config, input_):
        ...
        cost = get_loss_function(output, softmax_w, softmax_b,input_.targets, batch_size,num_steps, is_training)
        
        self._cost = cost = tf.reduce_sum(cost)
...
def get_loss_function(output, softmax_w, softmax_b,targets, batch_size, num_steps, is_training):
    if FLAGS.loss_function == "sequence_loss_by_example":
        ...
        for i in xrange(batch_size):
            ...
            batch = tf.cond(tf.equal(index[0,0],zero), lambda: batch, lambda: batch + 1)	    
            cost += loss
        return tf.cond(tf.equal(batch,zero), lambda: cost, lambda: cost/tf.cast(batch, dtype=data_type()))
...
def run_epoch(session, model, eval_op=None, verbose=False, epoch_nb = 0):
    ...
    for step in range(model.input.epoch_size):
        feed_dict = {}
        if step % model.input.steps_in_sentence == 0:                           # reset of state when starting a new sentence
            state = session.run(model.initial_state)

...
flags.DEFINE_integer('nb_topics', 0, 'nb_topics')
flags.DEFINE_integer('sentences_per_document', 0, 'sentences_per_document')
...
def main(_):
    ...
    if FLAGS.nb_topics == 0 and FLAGS.sentences_per_document == 0:
	lda_path = os.path.join(FLAGS.data_path, "lda.ds")
    	lda = models.LdaModel.load(lda_path)
    else:
	lda_path = os.path.join(FLAGS.data_path, 'topic_experiments/lda_'+str(FLAGS.nb_topics)+'_'+str(FLAGS.sentences_per_document)+'.ds')
    	lda = models.LdaModel.load(lda_path)

...
flags.DEFINE_integer("max_max_epoch", 20, "max_max_epoch")
...
flags.DEFINE_string("test_name","ptb_large_sampled","test_name")
...
def main(_):
    ...
    with tf.Graph().as_default():
        ...
        with tf.name_scope("train"):
            train_data = reader.ptb_data_continuous(config.batch_size, FLAGS.num_steps, FLAGS.data_path, train_name)
            with tf.variable_scope("model", reuse=None, initializer=initializer):
                m = ds_original_model(is_training=True, config=config, input_=train_data)

        with tf.name_scope("valid"):
            valid_data = reader.ptb_data_continuous(config.batch_size, FLAGS.num_steps, FLAGS.data_path, valid_name)
            with tf.variable_scope("model", reuse=True, initializer=initializer):
                mvalid = ds_original_model(is_training=False, config=config, input_=valid_data)

        with tf.name_scope("test"):
            test_data = reader.ptb_data_continuous(eval_config.batch_size, eval_config.num_steps, FLAGS.data_path, test_name)
            with tf.variable_scope("model", reuse=True, initializer=initializer):

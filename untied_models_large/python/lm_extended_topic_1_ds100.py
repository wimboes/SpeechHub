...
flags.DEFINE_integer("num_run", 1, "num_run")
...
def run_epoch(session, model, eval_op=None, verbose=False, epoch_nb = 0, pos_epoch = 0):
    ...
    for step in range(pos_epoch, model.input_sentence.epoch_size):
...
def main(_):
    print('job started')
    lda_path = os.path.join(FLAGS.data_path, "lda_100_100.ds")
...
######
        ...
        with sv.managed_session() as session:
            start_epoch = session.run(m.global_step) // m.input_sentence.epoch_size
            pos_epoch = session.run(m.global_step) % m.input_sentence.epoch_size
            print(pos_epoch)
            m.input_sentence.assign_batch_id(pos_epoch)         

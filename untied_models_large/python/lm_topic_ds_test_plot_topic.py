...
class ds_topic_model(object):
    def __init__(self, is_training, config, input_sentence, input_continuous, topic_matrix, initializer_reg, initializer_lda):
        ...
        self._cost, self._nb_words_in_batch, self._temp = get_loss_function(output_reg, output_lda, softmax_w_reg, softmax_w_lda, softmax_b_reg, softmax_b_lda, self._interpol, labels, topic_matrix, input_continuous, is_training)
...
def run_test_epoch(session, model, epoch_nb = 0):
    ...
    fetches = {"cost": model.cost, 'nb_words_in_batch': model.nb_words_in_batch, "final_state_reg": model.final_state_reg,"final_state_lda": model.final_state_lda, "temp" :model.temp}
    ...
    print(model.input_continuous.epoch_size)
    print(pos)
    plt.rc('text', usetex=True)
    plt.rc('font', family='serif')
    plt.plot(word_axis[:pos] +1, interpolation[:pos, 41],'y', label = 'weer') # np.arange(np.shape(interpolation)[0])
    plt.plot(word_axis[:pos] +1, interpolation[:pos, 86],'g', label = 'wielrennen')
    plt.plot(word_axis[:pos] +1, interpolation[:pos, 363],'b', label = 'voetbal')
    ...
    plt.savefig('topics_from_topic_model-eps-converted-to.eps')    

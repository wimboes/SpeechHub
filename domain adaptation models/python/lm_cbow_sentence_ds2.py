...
class ds_cbow_sentence_model(object):
    ...
    def __init__(self, is_training, config, input_):
        ...
        with tf.device("/cpu:0"):
            embedding_reg = tf.get_variable("embedding_reg", [vocab_size, config.embedded_size_reg], dtype=data_type())
            embedding_cbow = tf.get_variable("embedding_cbow", [vocab_size, config.embedded_size_cbow], dtype=data_type())
        ...
        with tf.variable_scope('cbow') as cbow:           
            ...
            for i in xrange(num_steps):
                ...
                if FLAGS.combination == "mean":
                    mask = tf.cast(tf.not_equal(slice1,[vocab_size]), dtype = data_type())
...
def main(_):
    ...
    with tf.Graph().as_default():
        ...
        with tf.name_scope("train"):
            train_data = reader.ds_data_with_history(config.batch_size, config.num_history, FLAGS.data_path, valid_name)
            with tf.variable_scope("model", reuse=None, initializer=initializer):
                m = ds_cbow_sentence_model(is_training=True, config=config, input_=train_data)

        with tf.name_scope("valid"):
            valid_data = reader.ds_data_with_history(config.batch_size, config.num_history, FLAGS.data_path, valid_name)
            with tf.variable_scope("model", reuse=True, initializer=initializer):
                mvalid = ds_cbow_sentence_model(is_training=False, config=config, input_=valid_data)

        with tf.name_scope("test"):
            test_data = reader.ds_data_with_history(eval_config.batch_size, eval_config.num_history, FLAGS.data_path, test_name)
            with tf.variable_scope("model", reuse=True, initializer=initializer):

...
class PTBInput(object):
	...
	def __init__(self, config, num_steps, data, length_array, name=None):
		self.batch_size = batch_size = config.batch_size
  		self.average_sentence_length = np.mean(length_array)
		self.epoch_size = (len(data) // batch_size)
		self.input_data, self.targets, self.num_steps_batch = reader_custom.ptb_producer(data, batch_size, num_steps, length_array, name=None)
...
class PTBModel(object):
    ...
    def __init__(self, is_training, config, input_):
        ...
        #inputs = [tf.squeeze(input_step, [1]) for input_step in tf.split(1, tf.squeeze(num_steps), inputs)]
        ...
        loss = get_loss_function(output, softmax_w, softmax_b,input_.targets, batch_size,tf.squeeze(num_steps), is_training)
...
def get_loss_function(output, softmax_w, softmax_b,targets, batch_size, num_steps, is_training):
    if FLAGS.loss_function == "full_softmax":
        ...
        cross_entropy = tf.nn.sparse_softmax_cross_entropy_with_logits(logits,tf.reshape(targets, [-1]))
        mask = tf.sign(tf.abs(tf.reshape(targets, [-1])))
        cross_entropy *= tf.cast(mask, dtype = data_type())
        cross_entropy = tf.reduce_sum(cross_entropy)
        cross_entropy /= tf.reduce_sum(tf.cast(mask, dtype = data_type()))
        return tf.reduce_mean(cross_entropy)
    if FLAGS.loss_function == 'sampled_softmax':
        if is_training:
            cross_entropy = tf.nn.sampled_softmax_loss(tf.transpose(softmax_w), softmax_b, output, tf.reshape(targets, [-1, 1]), 64, FLAGS.vocab_size)
            mask = tf.sign(tf.abs(tf.reshape(targets, [-1])))
            cross_entropy *= tf.cast(mask, dtype = data_type())
            cross_entropy = tf.reduce_sum(cross_entropy)
            cross_entropy /= tf.reduce_sum(tf.cast(mask, dtype = data_type()))
...
def main(_):
    ...
    raw_data = reader_custom.ptb_raw_data(FLAGS.data_path)
    train_data, valid_data, test_data, _, num_steps, train_length_array, valid_length_array, test_length_array = raw_data
    ...
    with tf.Graph().as_default():
        ...
        with tf.name_scope("Train"):
            train_input = PTBInput(config=config, num_steps=num_steps, data=train_data, length_array=train_length_array, name="TrainInput")
        ...
        with tf.name_scope("Valid"):
            valid_input = PTBInput(config=config, num_steps=num_steps, data=valid_data, length_array=valid_length_array, name="ValidInput")
            with tf.variable_scope("Model", reuse=True, initializer=initializer):
                mvalid = PTBModel(is_training=False, config=config, input_=valid_input)
            tf.scalar_summary("Validation Loss", mvalid.cost)

        with tf.name_scope("Test"):
            test_input = PTBInput(config=config,num_steps=num_steps, data=test_data, length_array=test_length_array, name="TestInput")
            with tf.variable_scope("Model", reuse=True, initializer=initializer):

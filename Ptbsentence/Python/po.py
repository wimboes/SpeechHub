...
def ptb_producer(raw_data, batch_size, num_steps,num_history, name=None):
    ...
    with tf.name_scope(name):
        ...
        z = tf.slice(history_data, [0, i*(num_steps+num_history-1)], [batch_size, tf.squeeze(tf.slice(lengths3,[i],[1]))])
        
        return data,x, y, tf.slice(lengths2,[i],[1]), z
...
train_data, valid_data, test_data, vocabulary, num_steps = a
data,x,y, num, z = ptb_producer(train_data, batch_size, num_steps, num_history)
...
[data1, l,p,q,r] = sess.run([data,x,y,num,z])
print(np.shape(l))
print(np.shape(p))
print(q)

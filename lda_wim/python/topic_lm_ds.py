...
flags.DEFINE_integer("hidden_size_reg", 512, "hidden_size_reg")
flags.DEFINE_integer("max_epoch_reg", 6, "max_epoch_reg")
flags.DEFINE_integer("max_max_epoch_reg", 2, "max_max_epoch_reg")
flags.DEFINE_float("keep_prob_reg", 0.5, "keep_prob_reg")
flags.DEFINE_float("lr_decay_reg", 0.8, "lr_decay_reg")
...
flags.DEFINE_integer("hidden_size_lda", 512, "hidden_size_lda")
flags.DEFINE_integer("max_epoch_lda", 6, "max_epoch_lda")
...
flags.DEFINE_integer("embedded_size_lda", 256, "embedded_size_lda")
...
class ds_input(object):
    def __init__(self, config, max_length, length_array, data, name=None):
        ...
        self.input_data, self.targets, self.sentence_lengths_batch = topic_reader_ds.ds_producer(data, batch_size, length_array, max_length, name=None)
...
class topic_model(object):
    def __init__(self, is_training, config, input_, topic_matrix, initializer_reg, initializer_lda):
        ...
        vocab_size = config.vocab_size #extra pad_symbool
...
def run_epoch(session, model, eval_op=None, verbose=False, epoch_nb = 0):
    ...
    fetches = {"cost": model.cost,"final_state_lda": model.final_state_lda, 'temp1':model.temp1}
...
def main(_):
    ...
    raw_data =  topic_reader_ds.ds_raw_data_lda(FLAGS.data_path)
    ...
    with tf.Graph().as_default():    
        ...
        with tf.name_scope("train"):
            ...
            with tf.variable_scope("model", reuse=None):
                m = topic_model(is_training=True, config=config, input_ = train_input, topic_matrix = topic_matrix, initializer_reg = initializer_reg, initializer_lda = initializer_lda)
        ...
        with tf.name_scope("valid"):
            ...
            with tf.variable_scope("model", reuse=True):
                mvalid = topic_model(is_training=False, config=config, input_ = valid_input, topic_matrix = topic_matrix, initializer_reg = initializer_reg, initializer_lda = initializer_lda)
        ...
        with tf.name_scope("test"):
            ...
            with tf.variable_scope("model", reuse=True):
                mtest = topic_model(is_training=False, config=eval_config, input_ = test_input, topic_matrix = topic_matrix, initializer_reg = initializer_reg, initializer_lda = initializer_lda) 
        ...
        with sv.managed_session() as session:
            if FLAGS.mode == 'reg':
                ...
                interpol_values = np.linspace(0,1,num=5)
                interpol_best = 0

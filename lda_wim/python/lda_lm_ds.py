...
class PTBInput(object):
    ...
    def __init__(self, config, data, name=None):
        ...
        self.input_data, self.targets = lda_reader_ds.lda_producer_ds(data, batch_size, num_steps)

class PTBModel(object):
    """The PTB model."""

    def __init__(self, is_training, config, input_, topic_matrix):
        self._input = input_
...
def get_loss_function(output, softmax_w, softmax_b, targets, topic_matrix, batch_size, num_steps, is_training):
    if FLAGS.loss_function == "full_softmax":
        logits = tf.matmul(output, softmax_w) + softmax_b
        topic_probs = tf.nn.softmax(logits) 
        output_probs = tf.matmul(topic_probs,topic_matrix)
        #target_indexing_tensor = tf.concat(1,[tf.transpose([tf.constant(range(batch_size*num_steps))]),tf.transpose([tf.reshape(targets, [-1])])])
        idx = tf.reshape(targets, [-1])
        idx_flattened = tf.range(0, tf.shape(output_probs)[0]) * tf.shape(output_probs)[1] + idx
...
def main(_):
    ...
    raw_data =  lda_reader_ds.lda_raw_data_ds(FLAGS.data_path)
    train_data, valid_data, test_data, vocab_size, topic_array = raw_data 
    ...
    with tf.Graph().as_default():    
        ...
        with tf.name_scope("Train"):
            ...
            with tf.variable_scope("Model", reuse=None, initializer=initializer):
                m = PTBModel(is_training=True, config=config, input_=train_input, topic_matrix = topic_matrix)
        ...
        with tf.name_scope("Valid"):
            ...
            with tf.variable_scope("Model", reuse=True, initializer=initializer):
                mvalid = PTBModel(is_training=False, config=config, input_=valid_input, topic_matrix = topic_matrix)
            tf.scalar_summary("Validation Loss", mvalid.cost)

        with tf.name_scope("Test"):
            test_input = PTBInput(config=eval_config, data=test_data, name="TestInput")
            with tf.variable_scope("Model", reuse=True, initializer=initializer):
                mtest = PTBModel(is_training=False, config=eval_config, input_=test_input, topic_matrix = topic_matrix)
        ...
        param_train_np = np.array([['init_scale',config.init_scale], ['learning_rate', config.learning_rate],
                                   ...
                                   ['max_max_epoch', config.max_max_epoch],['keep_prob', config.keep_prob], ['nb_topcis', nb_topics],

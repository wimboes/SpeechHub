...
class ds_original_model(object):
	...
	self._saver = tf.train.Saver({"embedding": embedding})	
...
def main(_):
    ...
    with tf.Graph().as_default():
        ...
        sv = tf.train.Supervisor(summary_writer=None,save_model_secs=3, logdir=FLAGS.save_path + '/' + FLAGS.test_name + '_' + str(FLAGS.num_run))
        with sv.managed_session() as session:
            ...
            m.saver.save(session,FLAGS.save_path + '/' + FLAGS.test_name + '_' + str(FLAGS.num_run)+ '/embeddings_' + str(train_data.pad_id+1) + '_' + str(config.embedded_size)  +'.ckpt')            
